...
import com.somnus.pay.core.pojo.Callback;
import com.somnus.pay.core.pojo.Callback.CallbackId;
import com.somnus.pay.core.pojo.PaymentOrder;
import com.somnus.pay.core.pojo.PaymentResult;
import com.somnus.pay.core.service.CallbackService;
import com.somnus.pay.core.service.PayService;
import com.somnus.pay.core.service.PaymentChannelSwitchService;
import com.somnus.pay.core.service.PaymentOrderService;
...
import com.somnus.pay.core.support.exceptions.UnsupportedRequestTypeException;
...
public abstract class PaymentChannelHandler {
	...
	public <R> R handle(RequestParameter<?, ?> parameter){
		...
		switch (type) {
			case RETURN:
				result = this.handleCallback((RequestParameter<?, String>) parameter);
				break;
			case NOTIFY:
				result = this.handleCallback((RequestParameter<?, String>) parameter);
				break;
	...
	protected String handleCallback(RequestParameter<?, String> parameter) {
		...
		if(parameter.getType() == RequestType.RETURN){
			...
			throw new PayException(msa.getMessage(MsgCodeList.ERROR_301007, new Object[]{parameter.getType()}));
		}
		Assert.isEmpty(result, "result must not be empty");
		try {
			...
			throw new PayException(msa.getMessage(MsgCodeList.ERROR_301006, new Object[]{parameter.getType()}));
		}
		return result;
	}
